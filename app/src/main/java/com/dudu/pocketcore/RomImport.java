...
public final class RomImport {
    ...
    private static void showFound(final Activity a, final List<File> found) {
        if (found.isEmpty()) {
            new AlertDialog.Builder(a)
                    .setTitle("스캔 결과")
                    .setMessage("저장소에서 롬(.ngc/.ngp/.npc)을 찾지 못했습니다.\n\n"
        ...
        for (int i = 0; i < found.size(); i++) {
            ...
            labels[i] = rel + String.format("  (%.1fMB)", f.length() / 1048576f)
                    + (dup ? " — 이미 있음" : "");
            checked[i] = !dup;
        }
        new AlertDialog.Builder(a)
                .setTitle("찾은 롬 " + found.size() + "개 — 가져올 것을 고르세요")
